...
var IconService = class _IconService {
  ...
  constructor(_rendererFactory, _handler, _document, sanitizer, _antIcons) {
    ...
    this._jsonpIconLoad$ = new import_rxjs.Subject();
  ...
  getRenderedContent(icon, twoToneColor) {
    ...
    const $iconDefinition = definition ? (0, import_rxjs.of)(definition) : this._loadIconDynamically(icon);
    return $iconDefinition.pipe((0, import_operators.map)((i) => {
      if (!i) {
        throw IconNotFoundError(icon);
      }
  ...
  _loadIconDynamically(type) {
    if (!this._http && !this._enableJsonpLoading) {
      return (0, import_rxjs.of)(HttpModuleNotImport());
    }
    ...
    if (!inProgress) {
      ...
      const source = !this._enableJsonpLoading ? this._http.get(safeUrl, {
        responseType: "text"
      }).pipe((0, import_operators.map)((literal) => __spreadProps(__spreadValues({}, icon), {
        icon: literal
      }))) : this._loadIconDynamicallyWithJsonp(icon, safeUrl);
      inProgress = source.pipe((0, import_operators.tap)((definition) => this.addIcon(definition)), (0, import_operators.finalize)(() => this._inProgressFetches.delete(type)), (0, import_operators.catchError)(() => (0, import_rxjs.of)(null)), (0, import_operators.share)());
  ...
  _loadIconDynamicallyWithJsonp(icon, url) {
    return new import_rxjs.Observable((subscriber) => {
      ...
      this._jsonpIconLoad$.pipe((0, import_operators.filter)((i) => i.name === icon.name && i.theme === icon.theme), (0, import_operators.take)(1)).subscribe((i) => {
        subscriber.next(i);
        clean();
      });
    });
...
var NzIconService = class _NzIconService extends IconService {
  ...
  constructor(rendererFactory, sanitizer, nzConfigService, platform) {
    ...
    this.configUpdated$ = new import_rxjs2.Subject();
...
var NzIconDirective = class _NzIconDirective extends IconDirective {
  ...
  changeIcon2() {
    ...
    this.ngZone.runOutsideAngular(() => {
      (0, import_rxjs2.from)(this._changeIcon()).pipe((0, import_operators2.takeUntil)(this.destroy$)).subscribe({
        next: (svgOrRemove) => {
          this.ngZone.run(() => {
...
//# sourceMappingURL=chunk-SVW5IIRM.js.map
